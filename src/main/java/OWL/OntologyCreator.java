...
//    if (!object.getKey().getLocalName().equals("Трансформатор")) {
 ...
 //           }
                ...
                case "Выключатель":
                    ...
                    List<Individual> protectionBRKlineIndividuals = new ArrayList<>();
                    Map<String, String> brkNames = breakerNames();
 //                   if (!object.getKey().getLocalName().equals("Трансформатор") && !(object.getKey().getLocalName().equals("ЛЭП"))) {
                    for (Map.Entry<String, String> name : brkNames.entrySet()) {
                        ...
                        for (OntClass protection : object.getValue()) {
                            Individual protIndividual = protection.createIndividual(nameSpace + name.getValue());
                            protectionBRKlineIndividuals.add(protIndividual);
                        }
                    }
 //                   }
                    objectIndividuals.put(brkIndividuals, protectionBRKlineIndividuals);
                    break;
            }
        }
